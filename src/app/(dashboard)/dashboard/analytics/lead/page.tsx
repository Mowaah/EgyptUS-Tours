...
export default function LeadReportsPage() {
  ...
  const { data: reportsData, isLoading } = useSWR(
    ["/admin/reports/leads", filterParams],
    () => fetchLeadsReports(filterParams),
    {
      ...
      <div className={`${styles.chartsGridHalf} ${styles.leadChartsGrid}`}>
        <div className={styles.leftColumn}>
          <LeadsBySourceChart 
            ...
            actions={<ExportButtons onCsvClick={() => downloadReportExport("leads", "leads_by_source", filterParams)} />} 
          />
          <LostLeadsAnalysis 
            data={reportsData?.lost_leads_analysis} 
            actions={<ExportButtons onCsvClick={() => downloadReportExport("leads", "lost_leads_analysis", filterParams)} />} 
          />
        </div>
        ...
        <div className={styles.rightColumn}>
          <AvgTimeToConvertChart 
            ...
            actions={<ExportButtons onCsvClick={() => downloadReportExport("leads", "avg_time_to_convert", filterParams)} />} 
          />
        </div>
      </div>
    </div>
