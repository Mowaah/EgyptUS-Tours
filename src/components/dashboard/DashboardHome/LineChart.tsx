...
import { pathFromPoints } from "./utils";
...
export default function LineChart({ lines, area = false }: LineChartProps) {
  ...
  const handlePointerMove = (e: React.PointerEvent<SVGSVGElement>) => {
    ...
    if (wrapRef.current) {
      ...
      const wrapX = e.clientX - wrapRect.left;
      const wrapY = e.clientY - wrapRect.top;
      setHoverData({ index, x: wrapX, y: wrapY });
    }
  };
    ...
    <div className={styles.chartWrap} ref={wrapRef}>
      ...
      <div style={{ position: 'relative', width: '100%', height: '100%', minHeight: 0 }}>
        ...
        {Array.from({ length: 6 }).map((_, index) => (
          <line key={`h-${index}`} x1="0" x2={width} y1={(index * height) / 5} y2={(index * height) / 5} className={styles.gridLine} />
        ))}
        {months.map((_, index) => (
          <line key={`v-${index}`} y1="0" y2={height} x1={(index * width) / 11} x2={(index * width) / 11} className={styles.gridLine} />
        ))}
      ...
      {/* Hover Circles (rendered as HTML to prevent SVG scaling distortion) */}
      {hoverData !== null && lines.map((line, i) => {
      ...
      {hoverData !== null && (
        <div 
          ...
          style={{
            left: Math.min(hoverData.x + 20, wrapRef.current ? wrapRef.current.offsetWidth - 120 : 0),
        ...
        >
          ...
          {lines.map(line => {
            ...
            return (
              <div key={line.name} style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px' }}>
      ...
      <div className={styles.months}>{months.map((month) => <span key={month}>{month}</span>)}</div>
    </div>
  );
}
