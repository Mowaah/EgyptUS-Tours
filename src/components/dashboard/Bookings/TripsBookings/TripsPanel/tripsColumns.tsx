...
export const getTripsPillStyle = (status: string) => {
  const map: Record<string, string> = {
    ...
    in_stay: styles.pillPending,
    in_hotel: styles.pillPending,
    "in stay": styles.pillPending,
    "in hotel": styles.pillPending,
...
export const tripsColumns: DataTableColumn<TripBookingRow>[] = [
  ...
  },
  {
    ...
    render: (row) => {
      ...
      return (
        <span className={getTripsPillStyle(label === "No Refunded Amount" ? "no_refunded_amount" : row.operational_status)}>
          <i aria-hidden />
          {label}
        </span>
      );
