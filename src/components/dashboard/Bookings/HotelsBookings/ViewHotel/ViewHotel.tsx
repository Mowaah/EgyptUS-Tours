...
export default function ViewHotel({ bookingId }: ViewHotelProps) {
  ...
  const refundSummary = React.useMemo(() => {
    ...
    const travelDate = payload.stay?.check_in_date || payload.stay?.start_date || payload.check_in_date || payload.start_date || new Date().toISOString();
  ...
  const operationalStatusLabel = React.useMemo(() => {
    ...
    if (op === "in_stay" || op === "in_hotel" || op === "in stay" || op === "in hotel" || op === "on_trip") {
      return "In Hotel";
    }
